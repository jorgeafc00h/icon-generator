...
export function GoogleSignIn({ onSuccess, onError, variant = 'default' }: GoogleSignInProps) {
  ...
  const handleAuthCallback = async (idToken: string) => {
    ...
    try {
      ...
      // Send ID token to the existing auth endpoint
      ...
      toast.success(`Welcome back, ${authResponse.name || 'User'}!`, { id: loadingToast })
  ...
  const handleSignIn = () => {
    ...
    // Use implicit flow with id_token (works without backend secret)
  ...
  if (variant === 'large') {
    return (
      <button
        ...
        className="w-full group relative overflow-hidden bg-white hover:bg-gray-50 text-gray-800 font-semibold py-4 px-6 rounded-xl border-2 border-gray-200 hover:border-blue-300 transition-all duration-300 shadow-lg hover:shadow-xl"
      >
  ...
  return (
    <button
      onClick={handleSignIn}
      className="flex items-center gap-3 bg-white hover:bg-gray-50 text-gray-800 font-semibold py-3 px-6 rounded-lg border-2 border-gray-200 hover:border-blue-300 transition-all duration-200 shadow-md hover:shadow-lg"
    >
